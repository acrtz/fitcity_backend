...
class QueryBuilder {
  ...
  build(options = {}) {
    ...
    let fragment = sql.fragment`
      ...
      ${this.compiledData.orderBy.length ? sql.fragment`order by ${sql.join(this.compiledData.orderBy.map(([expr, ascending]) => sql.fragment`${expr} ${Number(ascending) ^ Number(flip) ? sql.fragment`ASC` : sql.fragment`DESC`}`), ",")}` : ""}
  ...
  lock(type) {
    ...
    if (type === "cursorComparator") {
      ...
      this.compiledData[type] = this.data[type].map(([a, b]) => [callIfNecessary(a, getContext()), b]);
    } else if (type === "from") {
      if (this.data.from) {
