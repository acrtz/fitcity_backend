...
function omit(entity, permission) {
  ...
  if (omitSpec) {
    ...
    if (omitSpec.indexOf(READ) >= 0) {
      const bad = PERMISSIONS_THAT_REQUIRE_READ.find(p => omitSpec.indexOf(p) === -1);
      if (bad) {
        throw new Error(`Error when processing @omit for ${entity.kind} '${entity.name}' - we currently don't support '${bad}' when '${READ}' is forbidden, to solve this add '${bad}' to the @omit clause or use ‘@omit‘ to omit from everything`);
      }
    }
